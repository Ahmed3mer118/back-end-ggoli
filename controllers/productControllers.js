...
exports.getProducts = async (req, res) => {
    try {
        const products = await Product.find().populate('subcategory brand', "subcategory_name brand_name ")
...
exports.getProductsbyId = async (req, res) => {
    try {
        ...
        const productId = await Product.findOne({ routeProduct })
...
exports.updateProduct = async (req, res) => {
    try {
        ...
        const cartUpdateResult = await Cart.updateMany(
            { productId: updatedProduct._id, isPurchased: false },
            [
                {
                    $set: {
        ...
        console.log(`Cart items updated: ${cartUpdateResult.modifiedCount}`);

    
        return res.status(200).json({
            message: `Product with ID ${routeProduct} updated successfully.`,
...
exports.updateStatusProduct = async (req, res) => {
    try {
        ...
        const { routeProduct } = req.params;
        const product = await Product.findOne({ routeProduct });
        if (!product) {
            return res.status(404).json({ error: "Product not found" });
        }

        const newActive = await Product.findOneAndUpdate({routeProduct}, { isActive }, { new: true, select: "isActive" });
        if (isActive === false) {
